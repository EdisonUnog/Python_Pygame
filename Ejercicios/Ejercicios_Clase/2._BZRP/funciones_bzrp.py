...
def mostar_max_views():
    ...
    print(f"\n *Cantidad maxima de views: {maximo_views}")
    for x in lista_bzrp:
...
###########################_Funciones_Con_Sintaxis_###############################

#B. Recorrer la lista imprimiendo por consola el título de cada video
def mostrar_lista_temas_sintaxis(lista:list,clave=None,valor=None):
    if clave is None:
        for x in lista:
            print(f" *Titulo: {x['title']}")
    else:
...
#C. Recorrer la lista imprimiendo por consola el título de cada video junto a la cantidad de reproducciones del mismo
def mostrar_lista_temas_views_sintaxis(lista:list,clave=None,valor=None):
    if clave is None:
        for x in lista:
            print(f" *Titulo: {x['title']}, \n   *Views: {x['views']}")
    else:
...
def calcular_maximo(lista:list, clave:str)->int:
    ...
    if(type(lista) == list and len(lista) > 0 and type(clave) == str and len(clave) > 0):
        for x in lista:
            if flag_primero == True or x[clave] > maximo:
                maximo = x[clave]
                flag_primero = False
    return maximo

#FUNCION QUE CALCULA EL MIMINO
def calcular_minimo(lista:list, clave:str)->int:
    flag_primero = True
    if(type(lista) == list and len(lista) > 0 and type(clave) == str and len(clave) > 0):
        for x in lista:
            if flag_primero == True or x[clave] < minimo:
                minimo = x[clave]
                flag_primero = False
    return minimo

#D. Recorrer la lista y determinar cuál es la cantidad máxima de reproducciones (MÁXIMO)
def mostrar_max_views_sintaxis(lista:list):
    maximo_len = calcular_maximo(lista,'views')
    print(f"*Cantidad maxima de views: {maximo_len}")
    print(type(maximo_len))
    mostrar_lista_temas_sintaxis(lista,'views',maximo_len)

#E. Recorrer la lista y determinar cuál es la cantidad máxima de reproducciones (MINIMO)
def mostrar_min_views_sintaxis(lista:list):
    minimo_len =calcular_minimo(lista, 'views')
    print(f"*Cantidad minima de views: {minimo_len}")
    mostrar_lista_temas_sintaxis(lista,'views',minimo_len)