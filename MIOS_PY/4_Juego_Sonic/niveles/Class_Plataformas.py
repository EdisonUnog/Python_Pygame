import pygame
from niveles.Auxiliar_img import*
from niveles.Configuraciones import*

class Plataforma:
    def __init__(self, x, y,width, height, velocidad, movimiento_ms, left, right, velocidad_top_down, i, top, l_r=None):

        self.image_list= Auxiliar.cargarImagen("Recursos/Materiales/plataformas/plat{0}.png",21,False,1,1,w=width,h=height)
    ...
    def punto_volver(self):
        ...
        '''
        if(self.l_r == 0):
            if(self.rect.x >= self.punto_volver_plat_r and self.rect.y >= 600):
                self.move_r = False
                self.move_l = True
                self.move_down = False
            elif(self.rect.x <= self.punto_volver_plat_l and self.rect.y >= 600):
                self.move_x = 0
                self.move_r = False
                self.move_l = False
                self.move_up = True
