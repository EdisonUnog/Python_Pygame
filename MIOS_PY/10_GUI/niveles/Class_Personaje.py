...
class Personaje:
    def __init__(self, pantalla, tamaño, pos_inicial, velocidad, delta_ms) -> None:
        ...
        self.tasa_movimiento_delta = delta_ms
        self.tiempo_transcurrido  = 0
    ...
    def update(self, pantalla, lista_piso, lista_plataformas, lista_trampas):

        
        self.aplicar_gravedad(pantalla, lista_piso, lista_plataformas) #siempre va fuera del match
        self.lista_proyectiles.update(lista_plataformas)
        self.lista_proyectiles.update(lista_piso)
        self.leer_inputs()
        self.collision_pared(lista_piso)
    ...
    #----------------------------------------------------------------
    def aplicar_gravedad(self, pantalla,lista_piso, lista_plataformas): #salto, caida
        #salto
        ...
        for piso in lista_piso:
            if self.collision_rect_bottom.colliderect(piso.collision_rect_top):
                self.desplazamiento_y = 0
                self.esta_saltando = False
                self.un_salto = False
                self.rect_principal.bottom = piso.rect_principal.top 
                break
            #_Collision techo
            elif self.collision_rect_top.colliderect(piso.collision_rect_bottom):
        ...
        for plataforma in lista_plataformas:
            if self.collision_rect_bottom.colliderect(plataforma.collision_rect_top):
                self.desplazamiento_y = plataforma.mover_y
                self.esta_saltando = False
                self.un_salto = False
                self.rect_principal.bottom = plataforma.rect_principal.top + 5
                break
    
    #----------------------------------------------------------------
    def collision_pared(self, lista_piso):
        self.collision_l = False
        self.collision_r = False
        #_Collision paredes 
        for piso in lista_piso:
            if (self.collision_rect_left.colliderect(piso.collition_rect)) and not self.collision_l:
            ...
            if (self.collision_rect_right.colliderect(piso.collition_rect)) and not self.collision_r:
                self.rect_principal.x -= self.velocidad
                self.collision_rect_top.x -= self.velocidad
                self.collision_rect_bottom.x -= self.velocidad
                self.collision_rect_right.x -= self.velocidad
    ...
    def disparar(self):
        ...
        if(self.direccion == 0):
            self.lista_proyectiles.generar_balas(15, self.direccion, 50, 15, 30, 30)
        else:
            self.lista_proyectiles.generar_balas(15, self.direccion, -10, 15, 30, 30)
